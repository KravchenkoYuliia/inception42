...
This project aims to broaden your knowledge of system administration by using Docker. The project is done on Virtual Machine.
...
- accepts and processes HTTP(s) requests on the server and returns responses to clients
...
- WWW is a set of applications communicationg via the Web protocol called HTTP ( Hypertext Transfer Protocol )
	Client send smth to the server

- HTTPs - the same HTTP but secured with cryptographic protocol: SSL or TLS
- TLS is cryptographic protocol.
	Client send smth to the server - the web browser encrypts the information and only the server can decrypt this message because it contains a special key.   
	It's called the cryptographic encryption

### MariaDB
- one of the most popular database servers in the world
- by default, will listen on port 3306
